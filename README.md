...
src/
  ...
  data/
    ...
    mappe.js                     La Mappatura: suffix correspondences, their two roads, their notes, their false friends and their drills
...
- **La Mappatura** — suffix correspondences taught as rules rather than word lists,
  ...
  La Mappatura deliberately sits outside the Leitner queue; the reasoning is
...
- **Gli Articoli** — the permanent strand, and L'Officina's third workbench.
  ...
  A wrong answer is **located, not solved**, the same as La Mappatura — but three
  ...
  the rule for an item whose answer is `lo` *is* the answer. Like La Mappatura it
...
- **Falsi Amici** — L'Officina's fourth workbench, and the one that fills up
  ...
  What was missing until now was not the words but the **memory**: La Mappatura has
  ...
  meaning is collected — La Mappatura's other three trap drills bait with `citità`,
...
3. **Due items first inside decks** — the scheduler is in place but the
   ...
   it.
4. **Typed recall / production, everywhere else** — La Mappatura now types, and
   ...
   multiple-choice, which is the reason La Mappatura stays out of it.
