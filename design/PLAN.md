...
- `src/modules/officina/` — the L'Officina hub (design 07) and its bench roster.
  Three benches open something (the vocabulary deck, La Mappatura, Gli Articoli);
...
- Four module screens (vocab, grammar, conversations, stories) still in the **old postcard styling**.
  La Mappatura, the L'Officina hub and Gli Articoli are in the new one, per the rule
  in open question 4.
...
- **La Mappatura** — ✅ built ([#11](https://github.com/majakulpa/italian-app/pull/11)).
...
- **Gli Articoli** — ✅ built. The permanent strand: Polish has no articles and
  ...
  It stays out of the Leitner queue, and for a different reason from La Mappatura.
  La Mappatura's blocker is the interaction model: La Piazza is multiple-choice
  and La Mappatura types. An article item *is* multiple-choice, so the queue would
...
- **Falsi Amici** — ✅ built. The traps collected as you hit them: `colazione` ≠
  *kolacja*, `droga` ≠ *droga*, `firma` ≠ *firma*, `divano` ≠ *dywan*. It needed
  La Mappatura first, because La Mappatura is where most of them get generated — and
  La Mappatura's `trap` verdict is now the write that records a catch, so walking
  ...
  slid meaning is collected — La Mappatura's other three trap drills bait with
...
`officina` routes to it, La Mappatura is reached from the map rather than only
...
shape available. La Mappatura implements this in `modules/mappe/feedback.js`; reuse
