...
describe("App", () => {
  ...
  // La Mappatura has no district of its own — it is a bench inside L'Officina —
  ...
  it("opens La Mappatura from the switcher and returns to the city", async () => {
    const user = userEvent.setup();
    render(<App />);

    await user.click(screen.getByRole("button", { name: "Menu" }));
    await user.click(screen.getByRole("menuitem", { name: "La Mappatura" }));
  ...
  // Gli Articoli is the same shape as La Mappatura: a bench inside L'Officina
