...
// nothing behind it until now. La Mappatura has returned a `kind: "trap"` verdict
...
// first, reveal last) and it is also La Mappatura's own trap-drill shape, which
...
//
// Built in the La Città design system, like La Mappatura, the hub and Gli
...
// Pink is Polish everywhere in L'Officina — the Polish road in La Mappatura, the
...
function Drill({ set, onBack, onDone, onGrade, onCaught }) {
  ...
  // every answer. Same reasoning as La Mappatura's.
  const submit = (event) => {
    ...
    if (next.correct || next.last) {
      ...
      // "learning" — the same bar La Mappatura, Gli Articoli and the grammar
...
// link says which of the two it is about to do. Same shape as La Mappatura's.
