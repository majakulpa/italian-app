...
const STORIES_BY_LEMMA = (() => {
  ...
  for (const level of STORY_LEVELS) {
    for (const story of level.stories) {
      // A story can gloss the same word in three paragraphs. It is still one
      // place you met it, so the story is listed once.
      const seen = new Set();

      for (const paragraph of story.paragraphs) {
        for (const [word, meaning] of Object.entries(paragraph.gloss)) {
