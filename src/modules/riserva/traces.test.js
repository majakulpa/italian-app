...
describe("the two sources", () => {
  ...
  // Most of the list is ahead of the lessons — 600 lexicon entries against
  ...
  // Ranks 301–400 raised this from 59 to 78, ranks 401–500 raised it again to
  // 99, and ranks 501–600 raised it again to 115 — ordinary everyday words
  // like `governo`, `la legge`, `aeroporto`, `il cameriere`, `il ristorante`,
  // `zio`, `grazie`, `prego`, `per favore`, `biglietto`, `conto`, `orario`,
  // `passaporto` and `valigia` turned out to already be deck words or
  // glossed in a story, without either being sought out on purpose. That is
  // the traces bridge doing exactly what it is for.
  it("leaves most of the shipped lexicon with no trace, and that is the honest count", () => {
    ...
    expect(traced).toHaveLength(115);
