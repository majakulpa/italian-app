...
// 301–400 added `governo` and `legge`, ranks 401–500 added `aeroporto`,
// `cameriere`, `ristorante` and `zio`, and ranks 501–600 added `grazie`,
// `prego`, `per favore`, `biglietto`, `conto`, `orario`, `passaporto` and
// `valigia`, all already deck words, which is why the bridge-only figure
// below now reads 34 — so a learner who mastered every word, drill, dialogue
// and story the app shipped saw 1.6% and "20 / 2000 solid", and could not
// see more. The other vocabulary words are real Italian and worth learning;
// they were simply outside the population this figure is a percentage of.
//
...
// them, which today is 1–600: **73.3%, and 600 / 2000 solid.** Ranks 301–400
// moved it up from 66.1% and 300 / 2000 to 69.1% and 400 / 2000, ranks
// 401–500 moved it again to 71.4% and 500 / 2000, and ranks 501–600 moved it
// to 73.3% — Σ 1/r over 1–600, normalised the same way. That is not a
// coincidence in kind, even though the number moved: the sanity check higher
// up this file says the top 300 words are worth about two thirds of running
// text, and the ceiling is always that same sum arriving from the other
// side, just over however many ranks are seeded.
//
// 600 is also the door: the generated serial (design screens 13–17) opens at
// 600 solid words, and solid words can only come from this list, so this
// batch is what makes that door reachable in principle. Reachable is not
// open — the mechanism can drill every one of these 600 ranks, but no
// account has studied them yet, and that is still study, not seeding.
//
// What is left under it is the honest bottleneck, and it is still the one
// PLAN.md calls the real one: the list is 600 of a 2,000 target, so 1,400
// ranks have no word to drill. Raising the ceiling past 73.3% is a content
...
// Ranks that actually have an entry in the file yet — the first 600 of 2,000.
