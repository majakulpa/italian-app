...
describe("the round trip over a word held on two benches", () => {
  ...
  // Thirty-four lemmas overlap today — ranks 301–400 added `governo` and
  // `legge`, ranks 401–500 added `aeroporto`, `cameriere`, `ristorante` and
  // `zio`, and ranks 501–600 added `grazie`, `prego`, `per favore`,
  // `biglietto`, `conto`, `orario`, `passaporto` and `valigia`, all already
  // deck words — and the whole overlap is used rather than a hand-picked pair
  // because the bug scaled with it: 52 keys, a round of 26, and 26 still due
  // after answering every one of them correctly.
  ...
  // 34 pairs is now more than SESSION_LIMIT (20), so a single served round
